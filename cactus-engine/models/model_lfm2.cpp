...
bool LFM2Model::is_cache_empty() const {
    return cache_total_seq_len_ == 0;
}
void LFM2Model::update_kv_cache(CactusGraph* /*gb*/, size_t seq_len) {
    cache_total_seq_len_ += seq_len;
}
...
size_t LFM2Model::build_attention(CactusGraph* gb, size_t normalized_input, uint32_t layer_idx,
    ...
    if (use_cache && layer_idx < graph_cache_k_nodes_.size() && graph_cache_k_nodes_[layer_idx] != 0) {
        gb->kv_cache_append(k_proj_4d, graph_cache_k_nodes_[layer_idx], cache_window_size_, cache_sink_size_);
        gb->kv_cache_append(v_proj_4d, graph_cache_v_nodes_[layer_idx], cache_window_size_, cache_sink_size_);
        attn_output_4d = gb->attention_cached(
            q_proj_4d, k_proj_4d, v_proj_4d,
            graph_cache_k_nodes_[layer_idx], graph_cache_v_nodes_[layer_idx],
            attention_scale_, position_offset, cache_window_size_);
    } else {
...
size_t LFM2Model::forward(CactusGraph* gb, size_t input_embeddings, size_t seq_len,
    ...
    size_t position_offset = use_cache ? cache_total_seq_len_ : 0;
