...
int cactus_graph_sample(
    cactus_graph_t graph,
    cactus_node_t logits,
    float temperature,
    float top_p,
    size_t top_k,
    ...
    try {
        ...
        std::vector<int32_t> bitmask_values;
        if (bitmask) {
            bitmask_values.assign(bitmask, bitmask + bitmask_size);
        }
        *out = static_cast<cactus_node_t>(
            as_graph(graph)->graph.sample(
                static_cast<size_t>(logits),
                temperature,
                top_p,
                top_k,
                empty_bias,
